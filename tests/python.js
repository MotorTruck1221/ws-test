...
async function pythonTest() {
    ...
    await page.screenshot({path: `test-screenshots/python-chrome-basic.png`});
    console.log('Python screenshot (chrome, basic) taken');
    await chrome.close();
    ...
    await page2.screenshot({path: `test-screenshots/python-ff-basic.png`});
    console.log('Python screenshot (firefox, basic) taken');
    await fox.close();
    ...
    await page3.screenshot({path: `test-screenshots/python-chrome-binary.png`});
    console.log('Python screenshot (chrome, binary) taken');
    await chrome2.close();

    const fox2 = await firefox.launch({headless: false});
    const context4 = await fox2.newContext();
    const page4 = await context4.newPage();
    await page4.goto('http://localhost:8000/');
    for (let i = 0; i < 10; i++) {
        await page4.click('#sendFile');
    }
    await page4.screenshot({path: `test-screenshots/python-ff-binary.png`});
    console.log('Python screenshot (firefox, binary) taken');
    await fox2.close();
}
